...
def __load_extra_py_code_for_module(base, name, enable_debug_print=False):
    ...
    if native_module:
        ...
        for k, v in filter(
            lambda kv: not hasattr(py_module, kv[0]), native_module.__dict__.items()
        ):
            if enable_debug_print:
...
def bootstrap():
    ...
    if DEBUG:
        print(
            'OpenCV loader: os.name="{}"  platform.system()="{}"'.format(
                os.name, str(platform.system())
            )
        )
    ...
    def load_first_config(fnames, required=True):
        ...
        if required:
            raise ImportError(
                "OpenCV loader: missing configuration file: {}. Check OpenCV installation.".format(
                    fnames
                )
            )
    ...
    if DEBUG:
        print(
            "OpenCV loader: PYTHON_EXTENSIONS_PATHS={}".format(
                str(l_vars["PYTHON_EXTENSIONS_PATHS"])
            )
        )
    if DEBUG:
    ...
    if hasattr(sys, "OpenCV_REPLACE_SYS_PATH_0"):
        ...
        try:
            ...
            if DEBUG:
                print(
                    "OpenCV loader: exception during checking workaround for sys.path[0]"
                )
        ...
        if sys.version_info[:2] >= (
            ...
            for p in l_vars["BINARIES_PATHS"]:
                try:
                    ...
                    pass
        os.environ["PATH"] = (
            ";".join(l_vars["BINARIES_PATHS"]) + ";" + os.environ.get("PATH", "")
        )
        if DEBUG:
    ...
    for item_name, item in filter(
        lambda kv: kv[0]
        not in ("__file__", "__loader__", "__spec__", "__name__", "__package__"),
