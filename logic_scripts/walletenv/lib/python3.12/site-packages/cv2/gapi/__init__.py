...
def op(op_id, in_types, out_types):
    ...
    def op_with_params(cls):
        if not in_types:
            raise Exception(
                "{} operation should have at least one input!".format(cls.__name__)
            )

        if not out_types:
            raise Exception(
                "{} operation should have at least one output!".format(cls.__name__)
            )

        for i, t in enumerate(out_types):
            if t not in [cv.GMat, cv.GScalar, *garray_types, *gopaque_types]:
                raise Exception(
                    "{} unsupported output type: {} in position: {}".format(
                        cls.__name__, t.__name__, i
                    )
                )
